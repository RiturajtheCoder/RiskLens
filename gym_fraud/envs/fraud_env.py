...
class FraudEnv(gym.Env):
    ...
    def __init__(self, data_path='./dataset/creditcard.csv'):
        self.f = data_path
    ...
    def step(self, predicted_action_index):
        ...
        if self.turns > 10 or self.sum_rewards > 2 or self.current_state_index >= self.df_xy.shape[0] - 1:
    ...
    def _get_random_initial_state(self):
        nrand = random.randint(0, self.df_xy.shape[0] - 1)
    ...
    def _get_next_state(self):
        ...
        """
        df = self.df_xy
        new_state_index = min(self.current_state_index + 1, self.df_xy.shape[0] - 1)
